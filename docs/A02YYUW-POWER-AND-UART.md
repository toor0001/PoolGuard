...
| Gelb | RX | XIAO D1 / GPIO3 (Trigger-Ausgang) |
...
Die aktuelle Firmware implementiert **UART Controlled Output**: `A02 RX` ist mit
XIAO D1 / GPIO3 verbunden und wird nach dem Einschalten des Sensors wiederholt
mit fallenden Flanken getriggert. `A02 TX` liefert die Antwort über GPIO20. Der
Pololu bleibt bestehen und schaltet weiterhin die komplette A02-Versorgung
zwischen den Wake-Zyklen ab.

Die genaue Sensorvariante und das Controlled-UART-Verhalten müssen am realen
Prototyp weiterhin verifiziert werden. Bis dahin Gelb/RX nicht entfernen oder
fest für eine Auto-Output-Variante umverdrahten.
